...
function test( arr, fun, str ) {
    var testArr = paste(arr);
    var start = new Date().getTime();
    console.time(str);
    if ( str === "原生排序(似乎是快速排序)" ) {
        fun(testArr, ( a, b )=>a - b)
    } else {
        var _arr = fun(testArr)
...
function mergeArrays( arr, startLeft, stopLeft, startRight, stopRight ) {
    ...
    var _startRight = startRight;
    for ( let i = 0; i < rightArr.length - 1; ++i ) {
        rightArr[ i ] = arr[ _startRight ];
        ++_startRight;
    }
    var _startLeft = startLeft;
...
let testArr1 = generateRandomArr(100000);
var myTest1 = test.bind({}, testArr1);
